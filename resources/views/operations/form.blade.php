...
<form method="post" action="{{ $isEdit ? route('operations.update', [$module, $record->id]) : route('operations.store', $module) }}">
    ...
    <div class="form-grid">
        ...
        @elseif ($module === 'payments')
            ...
            <div><label>{{ __('Cash Account') }}</label><select class="searchable-select" name="cash_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($cashAccounts as $account)<option value="{{ $account->id }}" @selected(old('cash_account_id', $record?->cash_account_id) == $account->id)>{{ $account->localized_name }}</option>@endforeach</select></div>
            <div><label>{{ __('Bank Account') }}</label><select class="searchable-select" name="bank_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($bankAccounts as $account)<option value="{{ $account->id }}" @selected(old('bank_account_id', $record?->bank_account_id) == $account->id)>{{ $account->localized_name }}</option>@endforeach</select></div>
            ...
            <div><label>{{ __('Cheque Status') }}</label><input value="{{ __(ucfirst($record?->cheque_status ?? 'pending')) }}" readonly><input type="hidden" name="cheque_status" value="{{ $record?->cheque_status ?? 'pending' }}"><div class="muted">{{ __('Update settlement from Cheques In.') }}</div></div>
        @elseif ($module === 'stock-purchases')
