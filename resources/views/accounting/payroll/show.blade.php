...
<div class="section-title">
    ...
    <div style="display:flex;gap:8px">
        <a class="button" href="{{ route('accounting.salary-advances.index', ['year' => $payroll->period_year, 'month' => $payroll->period_month]) }}">{{ __('Salary Advances') }}</a>
...
<div class="kpi-grid">
    ...
    <div class="card kpi-card"><div class="muted">{{ __('Employee Social Security') }}</div><div class="kpi">{{ number_format($payroll->total_employee_social_security, 3) }}</div></div>
    <div class="card kpi-card"><div class="muted">{{ __('Employer Social Security') }}</div><div class="kpi">{{ number_format($payroll->total_employer_social_security, 3) }}</div></div>
    <div class="card kpi-card"><div class="muted">{{ __('Other Deductions') }}</div><div class="kpi">{{ number_format($payroll->total_deductions, 3) }}</div></div>
    <div class="card kpi-card"><div class="muted">{{ __('Social Security Payable') }}</div><div class="kpi">{{ number_format($payroll->total_social_security, 3) }}</div><div class="muted">{{ $payroll->social_security_paid_at ? __('Paid') : __('Unpaid') }}</div></div>
...
@endif
@if (in_array($payroll->status, ['posted', 'paid'], true) && ! $payroll->social_security_paid_at && $payroll->total_social_security > 0)
<div class="section-title"><div><h2>{{ __('Pay Social Security') }}</h2><div class="muted">{{ __('Settle employee and employer social security payable.') }}</div></div></div>
<form class="filters" method="post" action="{{ route('accounting.payroll.pay-social-security', $payroll) }}">
    @csrf
    <div><label>{{ __('Payment Date') }}</label><input type="date" name="social_security_payment_date" value="{{ now()->toDateString() }}"></div>
    <div><label>{{ __('Payment Type') }}</label><select name="social_security_payment_type"><option value="cash">{{ __('Cash') }}</option><option value="bank_transfer">{{ __('Bank Transfer') }}</option></select></div>
    <div><label>{{ __('Cash Account') }}</label><select class="searchable-select" name="social_security_cash_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($cashAccounts as $account)<option value="{{ $account->id }}">{{ $account->localized_name }}</option>@endforeach</select></div>
    <div><label>{{ __('Bank Account') }}</label><select class="searchable-select" name="social_security_bank_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($bankAccounts as $account)<option value="{{ $account->id }}">{{ $account->localized_name }}</option>@endforeach</select></div>
    <div><label>{{ __('Reference') }}</label><input name="social_security_payment_reference"></div>
    <div><button>{{ __('Pay Social Security') }} ({{ number_format($payroll->total_social_security, 3) }})</button></div>
</form>
@endif
<div class="table-wrap">
<table>
    <thead><tr><th>{{ __('Employee') }}</th><th>{{ __('Gross Salary') }}</th><th>{{ __('Allowances') }}</th><th>{{ __('Employee Social Security') }}</th><th>{{ __('Other Deductions') }}</th><th>{{ __('Employer Social Security') }}</th><th>{{ __('Net Salary') }}</th><th>{{ __('Advances') }}</th><th>{{ __('Remaining') }}</th></tr></thead>
    <tbody>@foreach ($payroll->lines as $line) @php($lineAdvances = (float) $payroll->advances->where('status', 'posted')->where('employee_id', $line->employee_id)->sum('amount'))<tr><td>{{ $line->employee->localized_name }}</td><td>{{ number_format($line->gross_salary, 3) }}</td><td>{{ number_format($line->allowances, 3) }}</td><td>{{ number_format($line->employee_social_security, 3) }}</td><td>{{ number_format($line->deductions, 3) }}</td><td>{{ number_format($line->employer_social_security, 3) }}</td><td>{{ number_format($line->net_salary, 3) }}</td><td>{{ number_format($lineAdvances, 3) }}</td><td><strong>{{ number_format(max(0, (float) $line->net_salary - $lineAdvances), 3) }}</strong></td></tr>@endforeach</tbody>
</table>
</div>
...
<table>
    ...
    @foreach ($payroll->advances->sortBy('payment_date') as $advance)
        <tr>
            ...
            <td>@if ($payroll->status === 'draft' && $advance->status === 'posted')<form method="post" action="{{ route('accounting.salary-advances.cancel', $advance) }}" onsubmit="return confirm('{{ __('Cancel this salary advance and reverse its journal?') }}')">@csrf<button>{{ __('Cancel') }}</button></form>@endif</td>
        </tr>
    @endforeach
    </tbody>
</table>
