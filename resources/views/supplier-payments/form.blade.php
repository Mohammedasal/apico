...
<form method="post" action="{{ $isEdit ? route('supplier-payments.update', $payment) : route('supplier-payments.store') }}">
    ...
    <div class="form-grid">
        ...
        <div><label>{{ __('Cash Account') }}</label><select class="searchable-select" name="cash_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($cashAccounts as $account)<option value="{{ $account->id }}" @selected(old('cash_account_id', $payment->cash_account_id) == $account->id)>{{ $account->localized_name }}</option>@endforeach</select></div>
        <div><label>{{ __('Bank Account') }}</label><select class="searchable-select" name="bank_account_id"><option value="">{{ __('Use Default') }}</option>@foreach ($bankAccounts as $account)<option value="{{ $account->id }}" @selected(old('bank_account_id', $payment->bank_account_id) == $account->id)>{{ $account->localized_name }}</option>@endforeach</select></div>
        ...
        <div><label>{{ __('Cheque Status') }}</label><input value="{{ __(ucfirst($payment->cheque_status ?? 'pending')) }}" readonly><input type="hidden" name="cheque_status" value="{{ $payment->cheque_status ?? 'pending' }}"><div class="muted">{{ __('Update settlement from Cheques Out.') }}</div></div>
